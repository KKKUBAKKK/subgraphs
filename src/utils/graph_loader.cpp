...
GraphLoader::loadAdjacencyMatrix(const std::filesystem::path& filePath) {
...
std::pair<Multigraph, Multigraph> GraphLoader::loadFromFile(const std::filesystem::path& filePath) {
    std::ifstream infile(filePath);
    if (!infile.is_open()) {
        throw std::runtime_error("Could not open file: " + filePath.string());
    }
    ...
    Multigraph g1(std::move(matrix1));
    Multigraph g2(std::move(matrix2));
...
void GraphLoader::saveToFile(const Multigraph& g1, const Multigraph& g2,
