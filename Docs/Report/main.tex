...
W celu weryfikacji poprawności i wydajności zaimplementowanych algorytmów przeprowadzono szereg testów eksperymentalnych. Testy obejmowały pomiary czasu wykonania algorytmów dla różnych rozmiarów grafów oraz porównanie jakości rozszerzeń znajdowanych przez poszczególne algorytmy.

\subsection{Środowisko testowe}

Testy przeprowadzono na komputerze z procesorem Apple M3 oraz 8GB pamięci RAM. Program został skompilowany z optymalizacjami (-O3) przy użyciu kompilatora Clang. Dla każdego testu mierzono czas wykonania algorytmu w milisekundach.
...
Algorytm Aproksymacyjny v1 wykazuje znacznie dłuższy czas wykonania (do 30 sekund dla największych grafów), podczas gdy wszystkie warianty v2 pozostają bardzo szybkie (10-30 ms).
...
Dla grafów rzadkich obserwujemy podobny wzorzec - algorytm v1 jest znacznie wolniejszy od wariantów v2.
...
Rysunek~\ref{fig:exact_subgraph_var} pokazuje, że algorytm dokładny dla grafu 4×8 potrzebuje:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Złożoność czasowa}: Algorytm dokładny wykazuje wykładniczą złożoność i jest praktycznie użyteczny tylko dla bardzo małych grafów (do 5 wierzchołków wzorca). Algorytmy aproksymacyjne są wielomianowe i skalują się dobrze.

    \item \textbf{Jakość rozszerzeń}: Algorytm dokładny znajduje optymalne rozwiązania. Wśród algorytmów aproksymacyjnych, v1 daje znacznie lepsze wyniki niż wszystkie heurystyki v2.

    \item \textbf{Kompromis czas-jakość}: Algorytm v1 oferuje najlepszy kompromis - jest znacznie szybszy od dokładnego (tysiące razy) przy zachowaniu dobrej jakości rozszerzeń. Algorytmy v2 są najszybsze, ale jakość rozszerzeń jest znacznie gorsza.

    \item \textbf{Rekomendacja}: Dla grafów wzorca do 5 wierzchołków warto użyć algorytmu dokładnego. Dla większych grafów zalecany jest algorytm aproksymacyjny v1 jako oferujący najlepszy stosunek jakości do czasu wykonania.
\end{enumerate}
