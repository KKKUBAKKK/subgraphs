...
\section{Aproksymacyjne minimalne rozszerzenie multigrafu - algorytm pierwszy}
...
\section{Aproksymacyjne minimalne rozszerzenie multigrafu - algorytm drugi}
\label{sec:minimalne_rozszerzenie_approx_2}

Algorytm implementuje metodę Murty'ego do znajdowania $j$ najlepszych rozwiązań problemu przypisania, z dwoma modyfikacjami: po pierwsze problem dotyczy przypisania $k$ wierszy mniejszego multigrafu do $N$ kolumn większego ($k \le N$), po drugie chcemy, żeby przypisania były różne pod względem zmapowanego podzbioru wierzchołków większego grafu. Do wyboru $j$ najlepszych rozwiązań używamy heurystycznej funkcji kosztu, która aproksymuje rzeczywisty koszt rozszerzenia multigrafu. 

Po zastosowaniu powyższych modyfikacji, algorytm Murty'ego znajduje $j$ najlepszych mapowań $k$ wierzchołków mniejszego grafu, do $N$ wierzchołków większego grafu, minimalizujących heurystyczny koszt rozszerzenia. Na koniec, kiedy mamy już $j$ najlepszych mapowań, po kolei dodajemy wszystkie brakujące krawędzie do większego grafu, tworząc jego rozszerzenie zawierające $j$ kopii mniejszego grafu, po drodze zliczając liczbę dodanych krawędzi.

\subsection{Notacja i Definicje}
...
W naszym przypadku interesuje nas tylko przypisanie pierwszych $k$ wierszy (reprezentujących $P$). Mapowanie $f$ jest iniekcją $f: \{1, \dots, k\} \to \{1, \dots, N\}$. Algorytm węgierski znajduje pełną permutację $\sigma$, z której my ekstrahujemy $f$ jako:
...
Definiujemy $H(M')$ jako funkcję rozwiązującą problem przypisania (algorytm węgierski) na macierzy $M'$. Rozwiązanie podproblemu z ograniczeniami jest realizowane przez modyfikację macierzy kosztów:
\begin{itemize}
...
\end{itemize}
Rozwiązujemy ten problem poprzez wywołanie algorytmu $H(M')$ na zmodyfikowanej macierzy $M'$, gdzie:
$$ M'[i, j] = 
\begin{cases} 
...
Cały algorytm aproksymacyjny składa się z dwóch faz. Faza 1 znajduje $n$ heurystycznych mapowań. Faza 2 oblicza na ich podstawie minimalne rozszerzenie.

Faza 1 opiera się na dwóch kluczowych procedurach pomocniczych: (1) Algorytm Węgierski, który rozwiązuje problem przypisania, oraz (2) Procedurze Rozwiązywania z Ograniczeniami , która jest dodatkiem do Algorytmu Węgierskiego.

\subsubsection{Procedura Pomocnicza: Algorytm Węgierski}

Wewnątrz procedur używana jest funkcja \texttt{HungarianAlgorithm}. Odpowiada ona klasycznemu \textbf{Algorytmowi Węgierskiemu}.

\begin{definition}[Algorytm Węgierski]
Algorytm ten rozwiązuje Liniowy Problem Przypisania (LAP) w czasie wielomianowym.
\begin{itemize}
    \item \textbf{Dane wejściowe:} Macierz kosztów $M$ (rozmiar $N \times N$).
    \item \textbf{Dane wyjściowe:} Permutacja $\sigma$ zbioru $\{1, \dots, N\}$ (lub równoważnie, zbiór $N$ par (wiersz, kolumna), np. $(\text{rows}, \text{cols})$), taka, że koszt całkowity $C = \sum_{i=1}^{N} M[i, \sigma(i)]$ jest zminimalizowany.
    \item \textbf{Złożoność obliczeniowa:} $\mathcal{O}(N^3)$.
\end{itemize}
W kontekście naszej analizy, jest to podstawowa operacja o koszcie $\mathcal{O}(N^3)$, na której budowane są bardziej złożone procedury.
\end{definition}

\subsubsection{Procedura Pomocnicza: Rozwiązywanie z Ograniczeniami}
    
Ten algorytm jest rozwinięciem \texttt{HungarianAlgorithm}, które modyfikuje macierz $M$ w celu uwzględnienia zabronionych wierzchołków $E$.

\begin{algorithm}[H]
    \caption{Procedura Pomocnicza - Rozwiązywanie z Ograniczeniami}
    \label{alg:solve_with_constraints}
    \begin{algorithmic}[1]
        \Require Oryginalna macierz kosztów $M$, zbiór zabronionych wierzchołków $E \subset V_G$, wielkość wzorca $k$
        \Ensure Para $($ heurystyczny koszt mapowania  $C$, mapowanie $f)$
        
        \State $M' \gets \Call{copy}{M}$ \Comment{Skopiuj oryginalną macierz kosztów}
        
        \For{\textbf{each} $v$ \textbf{in} $E$} \Comment{Zastosuj ograniczenia}
            \For{$i \gets 1$ \textbf{to} $k$} \Comment{Zablokuj mapowania z $P$ na wierzchołek $v \in G$}
                \State $M'[i, v] \gets \infty$
            \EndFor
        \EndFor
        
        \State $(\text{rows}, \text{cols}) \gets \Call{StandardAssignmentSolver}{M'}$ \Comment{Wywołaj Algorytm Węgierski}
        \If{koszt jest $\infty$}
            \State \Return $(\text{None}, \text{None})$
        \EndIf
        
        \State $f \gets \Call{ExtractMapping}{\text{rows}, \text{cols}, k}$ \Comment{Wyodrębnij mapowanie dla $k$ wierszy}
        
        \State $C \gets \Call{CalculateTrueCost}{M, f}$ \Comment{Oblicz koszt na oryginalnej macierzy $M$}
        \State \Return $(C, f)$
    \end{algorithmic}
\end{algorithm}

\subsubsection{Algorytm Główny Fazy 1: Wyszukiwanie Heurystyczne}

Algorytm Fazy 1 wykorzystuje kolejkę priorytetową $Q$ do przechowywania kandydatów na rozwiązania. Każdy element w $Q$ to krotka: $(C, E, f)$, gdzie $C$ to koszt mapowania $f$, a $E$ to zbiór zabronionych wierzchołków, który wygenerował to rozwiązanie.

\begin{algorithm}[H]
    \caption{Faza 1: Heurystyczne Wyszukiwanie $n$ Mapowań}
    \label{alg:heuristic_search}
    \begin{algorithmic}[1]
        \Require Macierz kosztów $M$, Liczba szukanych mapowań $n$, Liczba wierszy wzorca $k$
        \Ensure Lista $L_{sol}$ par $(C, f)$ zawierająca do $n$ najlepszych mapowań
        
        \State $L_{sol} \gets []$ \Comment{Inicjalizacja listy rozwiązań}
        \State $Q \gets \text{new PriorityQueue()}$ \Comment{Inicjalizacja kolejki priorytetowej}
        
        \State $(C_1, f_1) \gets \Call{solve\_with\_constraints}{M, \emptyset, k}$ \Comment{Rozwiąż problem bez ograniczeń}
        \If{$C_1 \neq \text{None}$}
            \State $Q.\text{push}((C_1, \emptyset, f_1))$
        \EndIf
        
        \While{$Q$ nie jest pusta \textbf{and} $|L_{sol}| < n$}
            \State $(C, E, f) \leftarrow Q.\text{pop}()$
            
            \State $L_{sol}.\text{append}((C, f))$ \Comment{Dodaj najlepsze bieżące rozwiązanie}
            
            \State \Comment{Partycjonowanie (Rozgałęzienie) - generowanie $k$ nowych podproblemów}
            \For{$i \gets 1$ \textbf{to} $k$}
                \State $v \gets f(i)$ \Comment{Pobierz wierzchołek $v \in G$ użyty w mapowaniu}
                \State $E_{new} \leftarrow E \cup \{v\}$ \Comment{Stwórz nowy zbiór zabronionych wierzchołków}
                
                \State $(C_{new}, f_{new}) \leftarrow \Call{solve\_with\_constraints}{M, E_{new}, k}$
                
                \If{$C_{new} \neq \text{None}$}
                    \State $Q.\text{push}((C_{new}, E_{new}, f_{new}))$
                \EndIf
            \EndFor
        \EndWhile
...
\subsubsection{Algorytm Główny Fazy 2: Zastosowanie Rozszerzeń}

Po znalezieniu $n$ mapowań, Faza 2 oblicza końcowe rozszerzenie.

\begin{algorithm}[H]
    \caption{Faza 2: Zastosowanie Rozszerzeń}
    \label{alg:apply_extension}
    \begin{algorithmic}[1]
        \Require Lista mapowań $L_{sol}$, Oryginalny graf $A_G$, Wzorzec $A_P$
        \Ensure Końcowa macierz $A_{final}$, Całkowity koszt $C_{total}$
        
        \State $A_{final} \gets \Call{copy}{A_G}$
        \State $C_{total} \gets 0$
        \For{\textbf{each} $(C, f)$ \textbf{in} $L_{sol}$}
            \For{\textbf{each} $(row, col)$ \textbf{in} $f$} \Comment{Iteruj przez mapowanie $f$}
                \State value $\gets \max(0, A_P[row, col] - A_G[row, col])$
                \State $C_{total} \gets C_{total} + value$ \Comment{Zlicz brakujące krawędzie}
                \State $A_{final}[row, col] \gets A_G[row, col] + value$ \Comment{Aktualizuj krawędź do wymaganego maksimum}
            \EndFor
        \EndFor
...
Należy zauważyć, że algorytm \ref{alg:heuristic_search} \textbf{nie jest} algorytmem Murty'ego. Jest to heurystyczny algorytm przeszukiwania zachłannego (best-first search), który nie gwarantuje znalezienia $n$ globalnie najlepszych rozwiązań. Nie gwarantuje on również, że znalezione mapowania $f_i$ będą używały różnych podzbiorów wierzchołków (choć kolejka priorytetowa promuje eksplorację różnych rozwiązań). Algorytm ten \emph{może} wielokrotnie zwrócić to samo mapowanie, jeśli zostanie ono wygenerowane przez różne gałęzie przeszukiwania o najniższym koszcie.

Poprawność algorytmu rozumiemy jako jego zdolność do zakończenia działania i zwrócenia $n$ mapowań oraz poprawnie obliczonego rozszerzenia dla \emph{tych} mapowań.

\begin{theorem}[Zakończenie Algorytmu]
Algorytm (Faza 1 + Faza 2) zawsze kończy działanie w skończonym czasie.
\end{theorem}
\begin{proof}
Faza 1 (Algorytm \ref{alg:heuristic_search}): Pętla `while` wykonuje się dokładnie $n$ razy (po jednym razie na każde rozwiązanie dodane do $L_{sol}$). W każdej iteracji wykonuje $k$ wywołań procedury \ref{alg:solve_with_constraints}. Każde wywołanie \ref{alg:solve_with_constraints} wywołuje \texttt{StandardAssignmentSolver} (Algorytm Węgierski), który jest algorytmem o skończonej, wielomianowej złożoności ($\mathcal{O}(N^3)$). Ponieważ $n$, $k$ i $N$ są skończone, Faza 1 kończy działanie.
Faza 2 (Algorytm \ref{alg:apply_extension}): Pętla `for` wykonuje się $n$ razy, a wewnętrzne pętle wykonują $k^2$ operacji. Jest to również skończona liczba kroków.
\end{proof}

\begin{theorem}[Poprawność Rozszerzenia]
Graf $A_{final}$ zwrócony przez Algorytm \ref{alg:apply_extension} zawiera podgrafy izomorficzne z $P$ dla wszystkich $n$ mapowań $f \in L_{sol}$ znalezionych w Fazie 1.
\end{theorem}
\begin{proof}
Dowód wynika z konstrukcji Fazy 2. Algorytm iteruje przez każde mapowanie $f_i \in L_{sol}$. Dla każdego $f_i$ i każdej wymaganej krawędzi $(u,v)$ w $A_P$, algorytm sprawdza krawędź $(g_u, g_v)$ w $A_{final}$. Jeśli krotność $A_{final}[g_u, g_v]$ jest mniejsza niż $A_P[u, v]$, jest ona aktualizowana do $A_P[u, v]$ (linia 12). Ponieważ krotności krawędzi są tylko zwiększane (nigdy zmniejszane), po zakończeniu pętli `for` dla $f_i$, wszystkie wymagane krawędzie dla $f_i$ są obecne. Ponieważ dzieje się to dla wszystkich $f \in L_{sol}$, ostateczny graf $A_{final}$ gwarantuje pokrycie dla wszystkich $n$ mapowań.
\end{proof}

\subsection{Analiza Złożoności}

Analizujemy koszt obu faz algorytmu.

\subsubsection{Złożoność Pamięciowa}
\begin{itemize}
    \item Macierze $M$, $M'$, $A_G$, $A_{final}$: $\mathcal{O}(N^2)$.
    \item Lista $L_{sol}$: Przechowuje $n$ mapowań, każde o rozmiarze $k$. Koszt: $\mathcal{O}(nk)$.
    \item Kolejka priorytetowa $Q$: W $i$-tej iteracji pętli `while`, $Q$ zawiera $i \cdot k$ kandydatów. Maksymalny rozmiar $Q$ to $\mathcal{O}(nk)$. Każdy element przechowuje zbiór $E$, który w najgorszym razie (głębokie drzewo) może mieć rozmiar $\mathcal{O}(N)$.
    Koszt kolejki: $\mathcal{O}(nk \cdot N)$.
\end{itemize}
\textbf{Całkowita złożoność pamięciowa: $\mathcal{O}(N^2 + n k N)$.}

\subsubsection{Złożoność Obliczeniowa (Czasowa)}

\begin{enumerate}
    \item \textbf{Koszt Fazy 1 (Algorytm \ref{alg:heuristic_search}):}
    \begin{itemize}
        \item Pętla `while` wykonuje się $n$ razy.
        \item W każdej iteracji pętli `while`:
            \item Jedna operacja `Q.pop()`: $\mathcal{O}(\log(nk))$.
            \item $k$ wywołań pętli `for`.
            \item Wewnątrz pętli `for`:
                \item Jedno wywołanie \Call{solve\_with\_constraints}{} (Algorytm \ref{alg:solve_with_constraints}).
                \item Koszt \Call{solve\_with\_constraints}{} jest zdominowany przez \texttt{StandardAssignmentSolver}, czyli $\mathcal{O}(N^3)$.
                \item Jedna operacja `Q.push()`: $\mathcal{O}(\log(nk))$.
        \item Całkowity koszt $i$-tej iteracji `while`: $\mathcal{O}(k \cdot (N^3 + \log(nk)))$.
    \end{itemize}
    Całkowity koszt Fazy 1 to $n$ (iteracje `while`) razy koszt wewnętrznej pętli:
    $$ C_{\text{Faza 1}} = \mathcal{O}(n \cdot k \cdot N^3) $$
    
    \item \textbf{Koszt Fazy 2 (Algorytm \ref{alg:apply_extension}):}
    \begin{itemize}
        \item Pętla `for` po $L_{sol}$ wykonuje się $n$ razy.
        \item Wewnętrzne pętle (po $u$ i $v$) wykonują $k^2$ operacji.
    \end{itemize}
    Całkowity koszt Fazy 2 to:
    $$ C_{\text{Faza 2}} = \mathcal{O}(n \cdot k^2) $$
\end{enumerate}

Całkowity koszt algorytmu to suma kosztów obu faz (oraz kosztu budowy macierzy $M$, $\mathcal{O}(N^2 + k^2 + kN)$):
$$ C_{\text{total}} = C_{\text{Budowa M}} + C_{\text{Faza 1}} + C_{\text{Faza 2}} $$
$$ C_{\text{total}} = \mathcal{O}(N^2 + k^2 + kN) + \mathcal{O}(n \cdot k \cdot N^3) + \mathcal{O}(n \cdot k^2) $$

Zakładając $N \ge k$, dominującym członem jest koszt Fazy 1.
\textbf{Całkowita złożoność obliczeniowa: $\mathcal{O}(n \cdot k \cdot N^3)$.}

% \subsection{Algorytm Główny}

% Cały algorytm aproksymacyjny składa się z dwóch faz. Faza 1 jest oparta na zmodyfikowanym algorytmie Murty'ego i znajduje $n$ najlepszych heurystycznych mapowań. Faza 2 oblicza na ich podstawie minimalne rozszerzenie.

% Poniżej znajduje się opis procedury pomocniczej, która znajduje pojedyncze rozwiązanie.

% \begin{algorithm}[H]
%     \caption{Procedura Pomocnicza - Rozwiązywanie z Ograniczeniami}
%     \label{alg:solve_with_constraints}
%     \begin{algorithmic}[1]
%         \Require Oryginalna macierz kosztów $M$, zbiór zabronionych $E$, wielkość wzorca $k$
%         \Ensure Para $(C, f)$
        
%         \State $M' \gets \Call{copy}{M}$ \Comment{Skopiuj oryginalną macierz kosztów}
        
%         \For{\textbf{each} $v$ \textbf{in} $E$} \Comment{Zastosuj ograniczenia}
%             \For($i \gets 1$ \textbf{to} $k$) \Comment{Zablokuj mapowania na dany wierzchołek}
%                 \State $M'[i, v] \gets \infty$
%             \EndFor
%         \EndFor
        
%         \State $(\text{rows}, \text{cols}) \gets \Call{StandardAssignmentSolver}{M'}$ \Comment{Wywołaj np. algorytm węgierski}
...
% Algorytm główny Fazy 1 wykorzystuje kolejkę priorytetową $Q$ do przechowywania kandydatów na rozwiązania. Każdy element w $Q$ to krotka: $(C, E, f)$, gdzie $C$ to koszt mapowania $f$, a $E$ to zbiór ograniczeń, które wygenerowały to rozwiązanie.

% \begin{algorithm}[H]
%     \caption{Zmodyfikowany Algorytm Murty'ego - Znajdowanie $n$ mapowań}
%     \label{alg:murty_k_best}
%     \begin{algorithmic}[1]
%         \Require Macierz kosztów $M$, Liczba szukanych mapowań $n$, Liczba wierszy wzorca $k$
%         \Ensure Lista $L_{sol}$ par $(C, f)$ zawierająca do $n$ najlepszych mapowań
...
%             \State $L_{sol}.\text{append}((C, f))$
            
%             \State $E_{accum} \leftarrow E$ \Comment{Akumulator zabronionych krawędzi}
            
%             \For{$i \gets 1$ \textbf{to} $k$}
%                 \State $E_{new} \leftarrow E \cup \{f(i)\}$ \Comment{Nowy zbiór zabronionych wierzchołków}
...
% \begin{theorem}[Zakończenie Algorytmu]
% Algorytm Murty'ego (Algorytm \ref{alg:murty_k_best}) zawsze kończy działanie w skończonym czasie.
% \end{theorem}
% \begin{proof}
% Liczba możliwych mapowań $f$ (iniekcji z $\{1..k\}$ do $\{1..N\}$) jest skończona i wynosi $P(N, k) = N! / (N-k)!$. Zbiór \texttt{seen\_mappings} ($U_{seen}$) gwarantuje, że każde unikalne mapowanie $f$ jest dodawane do $L_{sol}$ i partycjonowane co najwyżej raz. Ponieważ pętla \texttt{while} jest również ograniczona przez $n$ (\texttt{max\_solutions}), algorytm musi się zakończyć.
% \end{proof}
...
% W kroku 2.c.i (linii 23 algorytmu), algorytm generuje $k$ podproblemów $P_1, \dots, P_k$ na podstawie $f_1$.
% \end{proof}
...
% Ponieważ (1) partycjonowanie jest kompletne i rozłączne, (2) optymalne rozwiązanie każdego podproblemu $P_i$ (czyli $H(P_i)$) jest najlepszym kandydatem z tego podzbioru, oraz (3) kolejka priorytetowa $Q$ zawsze przechowuje najlepszych kandydatów ze wszystkich dotychczas wygenerowanych podzbiorów, to gdy \texttt{Q.pop()} zwraca $f_j$, musi to być rozwiązanie o globalnie $j$-tym najniższym koszcie spośród wszystkich możliwych rozwiązań.

% \subsection{Analiza Złożoności}

% Analizujemy koszt znalezienia $n$ najlepszych rozwiązań (ustawiając $\texttt{max\_solutions} = j = n$).

% \subsection{Złożoność Pamięciowa}

% \begin{itemize}
%     \item Macierz kosztów \texttt{cost\_matrix} (oryginalna i kopie): $\mathcal{O}(N^2)$.
%     \item Lista rozwiązań \texttt{solutions}: Przechowuje $n$ mapowań, każde o rozmiarze $k$. Koszt: $\mathcal{O}(n k)$.
%     \item Zbiór \texttt{seen\_mappings}: Przechowuje $n$ krotek o rozmiarze $k$. Koszt: $\mathcal{O}(n k)$.
%     \item Kolejka priorytetowa \texttt{queue}: W najgorszym razie, każde z $n$ znalezionych rozwiązań generuje $k$ nowych kandydatów. Rozmiar kolejki jest rzędu $\mathcal{O}(n k)$. Każdy element przechowuje ograniczenia $I$ i $E$, które mogą rosnąć do $\mathcal{O}(k)$.
%     Koszt kolejki: $\mathcal{O}(n k \cdot k) = \mathcal{O}(n k^2)$.
% \end{itemize}
% \textbf{Całkowita złożoność pamięciowa: $\mathcal{O}(N^2 + n k^2)$.}

% \subsection{Złożoność Obliczeniowa (Czasowa)}

% \begin{itemize}
%     \item Główna pętla \texttt{while} wykonuje się $\mathcal{O}(n)$ razy (ignorując duplikaty, których w praktyce jest niewiele w porównaniu do $n$).
%     \item Wewnątrz pętli, pętla \texttt{for} (partycjonowanie) wykonuje się $k$ razy.
%     \item Wewnątrz pętli \texttt{for} wywoływana jest funkcja \texttt{solve\_with\_constraints}.
%     \item Koszt \texttt{solve\_with\_constraints} jest zdominowany przez \texttt{StandardAssignmentSolver} (np. \texttt{linear\_sum\_assignment}), którego złożoność dla macierzy $N \times N$ wynosi $\mathcal{O}(N^3)$.
%     \item Operacje na kolejce priorytetowej (\texttt{push}/\texttt{pop}) mają koszt $\mathcal{O}(\log |Q|) = \mathcal{O}(\log(nk))$.
% \end{itemize}
% Całkowity koszt to suma kosztów $\mathcal{O}(n)$ kroków "wyjęcia" rozwiązania i $\mathcal{O}(n \cdot k)$ kroków "rozwiązania podproblemu".

% $$ C_{\text{solver}} = \underbrace{\mathcal{O}(n \cdot \log(nk))}_{\text{Zarządzanie kolejką}} + \underbrace{\mathcal{O}(n \cdot k \cdot N^3)}_{\text{Rozwiązywanie podproblemów}} $$

% Ponieważ $\mathcal{O}(N^3)$ jest znacznie większe niż $\mathcal{O}(\log(nk))$, ten drugi człon jest pomijalny.
% \textbf{Złożoność obliczeniowa (Faza 1 - Solver): $\mathcal{O}(n \cdot k \cdot N^3)$.}

% \subsection{Faza 2: Zastosowanie Rozszerzeń i Końcowa Analiza Złożoności}
% \label{sec:zastosowanie_rozszerzen}

% Powyższa analiza dotyczy kosztu Fazy 1 (procedury-solvera), która znajduje $n$ najlepszych mapowań heurystycznych. Pełen algorytm aproksymacyjny musi również wykonać Fazę 2.

% \begin{enumerate}
%     \item \textbf{Faza 1 (Znalezienie mapowań):} Wywołanie algorytmu \ref{alg:murty_k_best} z $n$. Koszt tej operacji, jak wykazano powyżej, wynosi:
%     $$ C_{\text{Faza 1}} = \mathcal{O}(n \cdot k \cdot N^3) $$
...
% $$ C_{\text{Faza 2}} = \sum_{i=1}^{n} (\text{koszt obliczenia } C_{ext}(f_i, A_{curr}) \text{ i aktualizacji } A_{curr}) $$
...
% Całkowity koszt Fazy 2 to:
% $$ C_{\text{Faza 2}} = \sum_{i=1}^{n} \mathcal{O}(k^2) = \mathcal{O}(n \cdot k^2) $$
