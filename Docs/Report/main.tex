...
Algorytm wykorzystuje kolejkę priorytetową $Q$ do przechowywania kandydatów na rozwiązania. Każdy element w $Q$ to krotka: $(C, I, E, f)$, gdzie $C$ to koszt mapowania $f$, a $I$ i $E$ to zbiory ograniczeń, które wygenerowały to rozwiązanie.

\begin{algorithm}[H]
    \caption{Algorytm Murty'ego}
    \label{alg:murty_k_best}
    \begin{algorithmic}[1]
        \Require Macierz kosztów $M$, Liczba szukanych mapowań $\text{max\_solutions}$, Liczba wierszy wzorca $k$
        \Ensure Lista $L_{sol}$ par $(C, f)$ zawierająca do $\text{max\_solutions}$ najlepszych mapowań
        
        \State $L_{sol} \gets []$ \Comment{Inicjalizacja listy rozwiązań}
        \State $U_{seen} \gets \emptyset$ \Comment{Inicjalizacja zbioru użytych mapowań}
        \State $Q \gets \text{new PriorityQueue()}$ \Comment{Inicjalizacja kolejki priorytetowej}
        
        \State $(C_1, f_1) \gets \Call{solve\_with\_constraints}{\emptyset, \emptyset}$ \Comment{Rozwiąż problem bez ograniczeń}
        \If{$C_1 \neq \text{None}$}
            \State $Q.\text{push}((C_1, \emptyset, \emptyset, f_1))$
        \EndIf
        
        \While{$Q$ nie jest pusta \textbf{and} $|L_{sol}| < \text{max\_solutions}$}
            \State $(C, I, E, f) \leftarrow Q.\text{pop}()$
            
            \If{$f \in U_{seen}$} \Comment{Pomiń duplikat}
                \State \textbf{continue}
            \EndIf
            \State $U_{seen}.\text{add}(f)$
            \State $L_{sol}.\text{append}((C, f))$
            
            \State $I_{accum} \leftarrow I$ \Comment{Akumulator wymuszonych krawędzi}
            
            \For{$i \gets 1$ \textbf{to} $k$}
                \State $(r, c) \leftarrow (i, f(i))$ \Comment{Krawędź do partycjonowania}
                \State $E_{new} \leftarrow E \cup \{(r, c)\}$ \Comment{Nowy zbiór zabronionych}
                
                \State $(C_{new}, f_{new}) \leftarrow \Call{solve\_with\_constraints}{I_{accum}, E_{new}}$
                
                \If{$C_{new} \neq \text{None}$}
                    \State $Q.\text{push}((C_{new}, I_{accum}, E_{new}, f_{new}))$
                \EndIf
                
                \State $I_{accum} \leftarrow I_{accum} \cup \{(r, c)\}$ \Comment{Przygotowanie do nast. iteracji}
            \EndFor
        \EndWhile
...
\subsection{Krok 2: Zastosowanie Rozszerzeń i Końcowa Analiza Złożoności}
