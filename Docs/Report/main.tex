...
Algorytm Węgierski~\cite{dynamic_hungarian} rozwiązuje problem przydziału: mając macierz kosztów $C$ o wymiarach $n \times n$, znajdź przypisanie każdego wiersza do dokładnie jednej kolumny, minimalizując sumaryczny koszt.

\begin{algorithm}[H]
    \caption{Algorytm Węgierski}
    \label{alg:hungarian}
    \begin{algorithmic}[1]
        \Require Macierz kosztów $C[n][n]$
        \Ensure Wektor przypisań $assignment[n]$, koszt całkowity $cost$

        \State \textit{// Krok 1: Redukcja wierszy i kolumn}
        \For{każdy wiersz $i$}
            \State $minVal \gets \min_j C[i][j]$
            \For{każda kolumna $j$}
                \State $C[i][j] \gets C[i][j] - minVal$
            \EndFor
        \EndFor

        \State \textit{// Krok 2: Inicjalizacja gwiazdek (starred zeros)}
        \State $starMatrix \gets$ macierz zer, $coveredCols \gets$ wektor false
        \For{każde zero $C[i][j] = 0$}
            \If{wiersz $i$ i kolumna $j$ nie są pokryte}
                \State $starMatrix[i][j] \gets true$
                \State $coveredCols[j] \gets true$
            \EndIf
        \EndFor

        \State \textit{// Krok 3: Główna pętla}
        \While{liczba pokrytych kolumn $< n$}
            \State Znajdź niepokryte zero, oznacz jako \textit{primed}
            \If{w wierszu tego zera jest \textit{starred} zero}
                \State Odkryj kolumnę \textit{starred}, pokryj wiersz
            \Else
                \State \textit{// Krok 4: Przebudowa ścieżki}
                \State Stwórz naprzemienną ścieżkę \textit{primed} $\leftrightarrow$ \textit{starred}
                \State Zamień \textit{starred} $\leftrightarrow$ \textit{primed} wzdłuż ścieżki
            \EndIf
            \If{brak niepokrytych zer}
                \State \textit{// Krok 5: Modyfikacja macierzy}
                \State $h \gets$ minimum niepokrytych elementów
                \State Dodaj $h$ do pokrytych wierszy
                \State Odejmij $h$ od niepokrytych kolumn
            \EndIf
        \EndWhile

        \State \textit{// Odczyt rozwiązania}
        \For{każdy wiersz $i$}
            \State $assignment[i] \gets$ kolumna z $starMatrix[i][j] = true$
        \EndFor
        \State \Return $assignment$, $\sum_i C_{orig}[i][assignment[i]]$
    \end{algorithmic}
\end{algorithm}

\textbf{Dowód poprawności:}
Algorytm zachowuje niezmiennik, że suma zredukowanych kosztów optymalnego przypisania pozostaje stała po każdej modyfikacji macierzy (Krok 5). Redukcja wierszy i kolumn nie zmienia optymalnego rozwiązania, gdyż dodanie/odjęcie stałej od wiersza lub kolumny przesuwa wszystkie przypisania o tę samą wartość. Algorytm kończy się, gdy znajdzie $n$ niezależnych zer (każde w innym wierszu i kolumnie), co odpowiada optymalnemu przypisaniu.

\textbf{Złożoność obliczeniowa:}
\begin{itemize}
    \item Krok 1 (redukcja): $\mathcal{O}(n^2)$
    \item Kroki 2-5: Każda iteracja głównej pętli pokrywa co najmniej jedną nową kolumnę lub modyfikuje macierz. Modyfikacja macierzy (Krok 5) może wystąpić co najwyżej $\mathcal{O}(n^2)$ razy. Każda iteracja to $\mathcal{O}(n^2)$.
    \item \textbf{Całkowita złożoność: $\mathcal{O}(n^3)$}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \texttt{HungarianAlgorithm} (linia 10): Wywołanie Algorytmu Węgierskiego~\cite{dynamic_hungarian} na macierzy $k \times k$ ma złożoność wielomianową $\mathcal{O}(k^3)$.
...
\section{Testy i wyniki eksperymentalne}
\label{sec:testy}

W celu weryfikacji poprawności i wydajności zaimplementowanych algorytmów przeprowadzono szereg testów eksperymentalnych. Algorytmy aproksymacyjne oparte są na podejściu wykorzystującym ścieżki długości $r$ do przybliżonego izomorfizmu podgrafów~\cite{lerp_algorithm}. Testy obejmowały pomiary czasu wykonania algorytmów dla różnych rozmiarów grafów oraz porównanie jakości rozszerzeń znajdowanych przez poszczególne algorytmy.

\subsection{Testy wydajnościowe algorytmów aproksymacyjnych}

Pierwsza seria testów dotyczy porównania wydajności wszystkich algorytmów aproksymacyjnych dla różnych typów grafów. Testowano 7 wariantów algorytmów: Aproksymacyjny v1 oraz Aproksymacyjny v2 z sześcioma różnymi heurystykami (stopień, kierunkowy, kierunkowy-ignoruj, histogram, struktura, zachłanny).

\subsubsection{Grafy gęste}

Na Rysunku~\ref{fig:approx_dense} przedstawiono wyniki dla grafów gęstych, gdzie gęstość krawędzi wynosi około 50\%. Rozmiar grafu wzorca rośnie od 5 do 50 wierzchołków, a rozmiar grafu celu jest trzykrotnie większy.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{01_approx_dense.png}
    \caption{Czas wykonania algorytmów aproksymacyjnych dla grafów gęstych. Oś X pokazuje rozmiary grafów (wzorzec→cel), oś Y pokazuje czas w milisekundach (skala logarytmiczna).}
    \label{fig:approx_dense}
\end{figure}

Algorytm Aproksymacyjny v1 wykazuje znacznie dłuższy czas wykonania (do 30 sekund dla największych grafów), podczas gdy wszystkie warianty v2 pozostają bardzo szybkie (10-30 ms). Widzimy też, że czas wykonania wszytkich algorytmów aproksymacyjnych rośnie wraz z rozmiarem grafów.

\subsubsection{Grafy rzadkie}

Rysunek~\ref{fig:approx_sparse} przedstawia wyniki dla grafów rzadkich z gęstością krawędzi około 15\%.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{02_approx_sparse.png}
    \caption{Czas wykonania algorytmów aproksymacyjnych dla grafów rzadkich.}
    \label{fig:approx_sparse}
\end{figure}

Dla grafów rzadkich obserwujemy podobny wzorzec - algorytm v1 jest znacznie wolniejszy od wariantów v2. Tutaj także możemy zaobserwować wzrost wraz z rozmiarem grafów. Widzimy też, że czas wykonania wszystkich algorytmów nie różni sie widocznie dla grafów rzadkich i gęstych.

\subsubsection{Grafy mieszane}

Rysunki~\ref{fig:approx_sparse_dense} i~\ref{fig:approx_dense_sparse} pokazują wyniki dla kombinacji grafów o różnych gęstościach.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{03_approx_sparse_dense.png}
    \caption{Algorytmy aproksymacyjne: rzadki graf wzorca + gęsty graf celu.}
    \label{fig:approx_sparse_dense}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{04_approx_dense_sparse.png}
    \caption{Algorytmy aproksymacyjne: gęsty graf wzorca + rzadki graf celu.}
    \label{fig:approx_dense_sparse}
\end{figure}

Na tych dwóch rysunkach widzimy podobny wzorzec - algorytm v1 jest znacznie wolniejszy od wariantów v2, ale dalej osiąga on akceptowalny czas wykonania. Warto też zauważyć, że czas nie różni się zauważalnie dla tych rysunków. Dla rysunku \ref{fig:approx_dense_sparse} widzimy jednak większe wachania w czasach wykonania algorytmu, co sugeruje, że oprócz rozmiarów na czas wykonania jednak ma wpływ także gęstość.

\subsubsection{Zmiana liczby szukanych podgrafów}

Rysunek~\ref{fig:approx_subgraph_var} przedstawia wpływ liczby szukanych podgrafów na czas wykonania. Test przeprowadzono dla ustalonego grafu (wzorzec: 10 wierzchołków, cel: 60 wierzchołków).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{05_approx_subgraph_var.png}
    \caption{Wpływ liczby szukanych podgrafów (1-10) na czas wykonania algorytmów aproksymacyjnych.}
    \label{fig:approx_subgraph_var}
\end{figure}
% TODO: wyjasnij czemy taki szum i nie rosnie liniowo. moze usunac albo poprawic
Czas wykonania rośnie liniowo z liczbą szukanych podgrafów dla wszystkich algorytmów.

\subsection{Porównanie algorytmu dokładnego z aproksymacyjnymi}

Druga seria testów porównuje algorytm dokładny z algorytmami aproksymacyjnymi. Ze względu na wykładniczą złożoność algorytmu dokładnego, testy przeprowadzono tylko dla małych grafów (wzorzec 1-5 wierzchołków).

\subsubsection{Grafy gęste - porównanie z algorytmem dokładnym}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{06_exact_dense.png}
    \caption{Porównanie wszystkich algorytmów dla małych grafów gęstych. Algorytm dokładny wykazuje wykładniczy wzrost czasu.}
    \label{fig:exact_dense}
\end{figure}

Na Rysunku~\ref{fig:exact_dense} widać dramatyczną różnicę w czasie wykonania. Algorytm dokładny dla grafu wzorca o 5 wierzchołkach potrzebuje około 1 sekundy, podczas gdy algorytmy aproksymacyjne wykonują się w 3-5 ms.

\subsubsection{Grafy rzadkie - porównanie z algorytmem dokładnym}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{07_exact_sparse.png}
    \caption{Porównanie wszystkich algorytmów dla małych grafów rzadkich.}
    \label{fig:exact_sparse}
\end{figure}

Dla grafów rzadkich algorytm dokładny także jest znacznie wolniejszy od wariantów aproksymacyjnych, a jego czas wykonania rośnie wykładniczo z rozmiarem grafu. Widzimy, że dla bardzo małych grafów (1, 2, 3 wierzchołki) algorytm dokładny bardzo szybki, ale są to tak małe przypadki, że nie musi on wykonywać prawie żadnych obliczeń. Dla trochę większych grafów zaczynamy już widzieć odpowiednie zależności. Wachania w wynikach algorytmów aproksymacyjnych mogą być spowodowane danymi wejściowymi, ale są one na tyle małe, że można je zignorować. Mogą one być spowodowane również szumem towarzyszącym wokonaniu algorytmów i innych zadań na naszej maszynie

\subsubsection{Grafy mieszane - porównanie z algorytmem dokładnym}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{08_exact_sparse_dense.png}
    \caption{Wszystkie algorytmy: rzadki wzorzec + gęsty cel (małe grafy).}
    \label{fig:exact_sparse_dense}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{09_exact_dense_sparse.png}
    \caption{Wszystkie algorytmy: gęsty wzorzec + rzadki cel (małe grafy).}
    \label{fig:exact_dense_sparse}
\end{figure}

Na rysunkach~\ref{fig:exact_sparse_dense} i~\ref{fig:exact_dense_sparse} widzimy, że algorytm dokładny jest znacznie wolniejszy od wariantów aproksymacyjnych, a jego czas wykonania rośnie wykładniczo z rozmiarem grafu. Wnioski są podobne do tych z poprzednich rysunków.

\subsubsection{Zmiana liczby podgrafów - algorytm dokładny}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{10_exact_subgraph_var.png}
    \caption{Wpływ liczby szukanych podgrafów na czas wykonania. Algorytm dokładny wykazuje wykładniczy wzrost (3→145 sekund dla 1-3 podgrafów).}
    \label{fig:exact_subgraph_var}
\end{figure}

Rysunek~\ref{fig:exact_subgraph_var} pokazuje, że algorytm dokładny dla grafu wzorca 4 i celu 8 potrzebuje:
\begin{itemize}
    \item 1 podgraf: 5.8 ms
    \item 2 podgrafy: 297 ms
    \item 3 podgrafy: 144.7 sekundy (prawie 2.5 minuty)
\end{itemize}

Algorytmy aproksymacyjne wykonują się w czasie 3-4 ms niezależnie od liczby podgrafów.

Z tych obserwacji wynika, że na algorytm dokładny bardzo duży wpływ ma liczba szukanych podgrafów. Zgadza się to z naszymi założeniami.

\subsection{Jakość rozszerzeń}

Oprócz czasu wykonania, kluczowym kryterium oceny algorytmów jest jakość znajdowanych rozszerzeń, czyli liczba krawędzi, które należy dodać do grafu celu.

\subsubsection{Test na małym grafie}

Rysunek~\ref{fig:extension_quality} przedstawia porównanie jakości rozszerzeń dla małego grafu testowego (wzorzec: 6 wierzchołków, cel: 8 wierzchołków).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{extension_quality.png}
    \caption{Jakość rozszerzeń: liczba dodanych krawędzi dla każdego algorytmu (mniej = lepiej). Algorytm dokładny znajduje optymalne rozwiązanie.}
    \label{fig:extension_quality}
\end{figure}

Wyniki pokazują, że:
\begin{itemize}
    \item \textbf{Algorytm dokładny} znajduje optymalne rozwiązanie (3 i 5 krawędzi dla 1 i 2 podgrafów)
    \item \textbf{Aproksymacyjny v1} daje wyniki bliskie optymalnym (3 i 7 krawędzi)
    \item \textbf{Heurystyki v2} dają gorsze wyniki (6-9 krawędzi dla 1 podgrafu)
\end{itemize}

\subsubsection{Test na dużym grafie}

Rysunek~\ref{fig:extension_quality_large} przedstawia porównanie jakości rozszerzeń dla większego grafu (wzorzec: 10 wierzchołków, cel: 30 wierzchołków), gdzie algorytm dokładny nie mógł być uruchomiony ze względu na zbyt długi czas wykonania.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{extension_quality_large.png}
    \caption{Jakość rozszerzeń dla dużego grafu (tylko algorytmy aproksymacyjne). Aproks. v1 daje zdecydowanie najlepsze wyniki.}
    \label{fig:extension_quality_large}
\end{figure}

Dla większych grafów różnice między algorytmami są jeszcze bardziej widoczne:
\begin{itemize}
    \item \textbf{Aproksymacyjny v1}: 16-79 krawędzi (dla 1-5 podgrafów)
    \item \textbf{v2 (kier. ign.)}: 57-101 krawędzi (najlepszy z heurystyk v2)
    \item \textbf{Pozostałe v2}: 56-159 krawędzi
\end{itemize}

\subsection{Wnioski z testów}

\begin{enumerate}
    \item \textbf{Złożoność czasowa}: Algorytm dokładny wykazuje wykładniczą złożoność i jest praktycznie użyteczny tylko dla małych grafów. Algorytmy aproksymacyjne są wielomianowe i skalują się dobrze. Z aproksymacyjnych v1 i v2 szybsze są algorytmy v2, ale kosztem skuteczności.

    \item \textbf{Jakość rozszerzeń}: Algorytm dokładny znajduje optymalne rozwiązania. Wśród algorytmów aproksymacyjnych, v1 daje znacznie lepsze wyniki niż wszystkie heurystyki v2. Z heurystyk v2 dla mniejszych grafów i mniejszej ilości podgrafów nie widać większych różnic w jakości. Jednak dla większych grafów i większej ilości podgrafów algorytm v2 daje zdecydowanie najlepsze wyniki dla heurystyki bazującej na liczbie wchodzących i wychodzących krawędzi z ignorowaniem ich nadmiaru w grafie celu.

    \item \textbf{Kompromis czas-jakość}: Algorytm v1 oferuje najlepszy kompromis - jest znacznie szybszy od dokładnego przy zachowaniu dobrej jakości rozszerzeń. Algorytmy v2 są najszybsze, ale jakość rozszerzeń jest znacznie gorsza.
\end{enumerate}
